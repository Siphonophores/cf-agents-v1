...
// No need for custom WebSocket manager - we'll use Vercel AI SDK with AI Gateway via HTTP

// Configure OpenAI provider with AI Gateway support (following Cloudflare's official documentation)
...
export class Chat extends AIChatAgent<Env> {
  ...
  constructor(state: DurableObjectState, env: Env) {
    super(state, env);
    console.log("Chat agent instance created with AI Gateway integration");
  }
  ...
  async onChatMessage(
    ...
    console.log("onChatMessage called - processing chat request with AI Gateway");
    ...
    const dataStreamResponse = createDataStreamResponse({
      execute: async (dataStream) => {
        ...
        // Stream the AI response using GPT-4 via AI Gateway
...
`,
          ...
          tools: allTools,
          onFinish: async (args) => {
            console.log("streamText finished successfully via AI Gateway");
            onFinish(
